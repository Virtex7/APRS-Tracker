...
/* Anschlusslegende LCD Modul:
 * LCD	Arduino STM32		BIT
 * D0	D8	PA9		x
 * D1	D9	PC7		x
 * D2	D2	PA10		x
 * D3	D3	PB3		x
...
#define LCD_CS_PIN  GPIO0	// PB0 -> A3 // Chip Select goes to Analog 3
#define LCD_CD_PIN  GPIO4	// PA4 -> A2 // Command/Data goes to Analog 2
#define LCD_WR_PIN  GPIO1	// PA1 -> A1 // LCD Write goes to Analog 1
#define LCD_RD_PIN  GPIO0	// PA0 -> A0 // LCD Read goes to Analog 0
#define LCD_RST_PIN GPIO1	// PC1 -> RESET

#define LCD_CS_GPIO_PORT  GPIOB
#define LCD_CS_HIGH()     gpio_set(LCD_CS_GPIO_PORT, LCD_CS_PIN)
#define LCD_CS_LOW()      gpio_clear(LCD_CS_GPIO_PORT, LCD_CS_PIN)

#define LCD_RD_GPIO_PORT  GPIOA
#define LCD_RD_HIGH()     gpio_set(LCD_RD_GPIO_PORT, LCD_RD_PIN)
#define LCD_RD_LOW()      gpio_clear(LCD_RD_GPIO_PORT, LCD_RD_PIN)

#define LCD_WR_GPIO_PORT  GPIOA
#define LCD_WR_HIGH()	  gpio_set(LCD_WR_GPIO_PORT, LCD_WR_PIN)
#define LCD_WR_LOW()      gpio_clear(LCD_WR_GPIO_PORT, LCD_WR_PIN)

#define LCD_CD_GPIO_PORT  GPIOA
#define LCD_CD_HIGH()     gpio_set(LCD_CD_GPIO_PORT, LCD_CD_PIN)
#define LCD_CD_LOW()      gpio_clear(LCD_CD_GPIO_PORT, LCD_CD_PIN)

#define LCD_RST_GPIO_PORT GPIOC
#define LCD_RST_HIGH()    gpio_set(LCD_RST_GPIO_PORT, LCD_RST_PIN)
#define LCD_RST_LOW()     gpio_set(LCD_RST_GPIO_PORT, LCD_RST_PIN)
...
void gpio_init(void) {
	...
	gpio_mode_setup(GPIOA, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO0 | GPIO1 | GPIO4 | GPIO8 | GPIO9 | GPIO10);

	/*Configure GPIO pins : PB0 PB3 PB4 PB5 PB10 */
	gpio_mode_setup(GPIOB, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO0 | GPIO3 | GPIO4 | GPIO5 | GPIO10);
	
	/*Configure GPIO pins : PC1 PC7 */
	gpio_mode_setup(GPIOC, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO1 | GPIO7);
}
...
void LCD_Write8(uint8_t data)
{
	...
	gpio_port_write(GPIOA, (gpio_port_read(GPIOA) & 0xF8FF) | ((data & 0x01) << 9) | ((data & 0x04) << 8) | ((data & 0x80) << 1));
	gpio_port_write(GPIOB, (gpio_port_read(GPIOB) & 0xFBC7) | (data & 0x08) | ((data & 0x10) << 1) | ((data & 0x20) >> 1) | ((data & 0x40) << 4));
	gpio_port_write(GPIOC, (gpio_port_read(GPIOC) & 0xFF7F) | ((data & 0x02) << 6));
